...
export function FileExplorer() {
  ...
  // Whether a file is selected for preview
  const hasPreview = createMemo(() => fileStore.openTabs.active !== null);

  // Tree/preview split sizing
  const [treeHeight, setTreeHeight] = createSignal(40); // percentage
    ...
    <div class="flex h-full flex-col border-l border-gray-200 bg-white dark:border-zinc-800 dark:bg-zinc-900">
      ...
      <div class="relative flex min-h-0 flex-1 flex-col">
        ...
        <div
          ...
          style={{
            height: hasPreview() ? `${treeHeight()}%` : "100%",
            "min-height": hasPreview() ? "120px" : undefined,
          }}
        >
          <Show
          ...
          >
            {/* Three-state check: loading → empty → tree */}
            <Show
              when={!rootLoading()}
              fallback={
        ...
        <Show when={hasPreview()}>
          <ResizeHandle
            ...
            min={15}
            max={85}
            onResize={(pct) => setTreeHeight(pct)}
          />
