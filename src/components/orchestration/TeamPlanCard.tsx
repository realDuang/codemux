...
import type { OrchestrationRun, OrchestrationSubtask, OrchestratorRole } from "../../types/unified";
import { getRoleMappings } from "../../stores/orchestration";
...
export function TeamPlanCard(props: TeamPlanCardProps) {
  ...
  const handleRoleChange = (subtaskIndex: number, role: OrchestratorRole) => {
    const mapping = getRoleMappings().find((m) => m.role === role);
    if (!mapping) return;
    setSubtasks((prev) => {
      const next = [...prev];
      next[subtaskIndex] = {
