...
GitHub Copilot은 세계에서 가장 널리 사용되는 AI 코딩 도구입니다. **Copilot CLI**는 터미널에서 완전한 에이전트 기능을 제공하지만, 이를 위한 그래픽 인터페이스는 존재하지 않았습니다.
...
[Feishu](https://www.feishu.cn/)에서 직접 AI 코딩 에이전트를 사용하세요 — 브라우저가 필요 없습니다. CodeMux는 Feishu 봇으로 연결되어 채팅 메시지를 게이트웨이를 통해 모든 엔진에 브리징합니다.

**1그룹 = 1세션**: 각 Feishu 그룹 채팅은 하나의 CodeMux 세션에 매핑됩니다. 봇의 P2P 채팅에서 프로젝트를 선택하면 세션용 그룹이 자동 생성되어, 대화가 분리되고 집중할 수 있습니다.

**봇 메뉴와 슬래시 명령**으로 Feishu 안에서 완전히 제어 가능합니다:

| 명령 | 컨텍스트 | 기능 |
|------|---------|------|
| 메뉴: 프로젝트 전환 | P2P | 프로젝트 탐색 및 선택 |
| 메뉴: 새 세션 | P2P | 새 세션 생성 (이전에 사용한 프로젝트가 있으면 프로젝트 선택 생략) |
| 메뉴: 세션 전환 | P2P | 기존 세션 간 전환 |
| `/cancel` | 그룹 | 현재 AI 응답 중지 |
| `/mode <agent\|plan\|build>` | 그룹 | 실행 모드 전환 |
| `/model list` / `/model <id>` | 그룹 | 모델 목록 보기 또는 전환 |
| `/status` | 그룹 | 세션 정보 표시 |

AI 응답은 스로틀된 메시지 업데이트로 실시간 스트리밍되며, 완료 시 도구 요약(예: `Shell(2), Edit(1)`)이 추가됩니다.

> **설정**: Feishu 개방 플랫폼에서 봇 기능이 있는 커스텀 앱을 생성하고, **WebSocket(장기 연결)** 모드로 이벤트 구독을 활성화한 후, CodeMux에서 App ID / App Secret을 설정하세요. 자세한 내용은 [Feishu 개방 플랫폼](https://open.feishu.cn/)을 참조하세요.
...
- [Feishu 개방 플랫폼](https://open.feishu.cn/) — Feishu 봇 연동
