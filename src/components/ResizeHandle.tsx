...
export function ResizeHandle(props: ResizeHandleProps) {
  const [local, rest] = splitProps(props, [
    "direction", "edge", "size", "min", "max",
    "onResize", "onCollapse", "collapseThreshold", "class",
  ]);
    ...
    const onMouseMove = (moveEvent: MouseEvent) => {
      ...
      const delta =
        local.direction === "vertical"
          ? edge === "end" ? pos - start : start - pos
          : edge === "start" ? start - pos : pos - start;
  ...
  return (
    <div
      ...
      class={`resize-handle ${local.class ?? ""}`}
